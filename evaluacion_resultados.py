...
class EvaluadorResultados:
    @staticmethod
    def registrar_prediccion(fecha, local, visita, cuota, prob, ev, kelly, recomendacion, goles_l=None, goles_v=None, marcador=None):
        """Registra la predicción en el historial para su posterior evaluación."""
        ...
        if os.path.exists(HISTORIAL_FILE):
            ...
            if not ((df['Fecha_Partido'] == fecha) & (df['HomeTeam'] == local) & (df['AwayTeam'] == visita)).any():
    ...
    @staticmethod
    def conciliar_resultados():
        """Cruza el historial_apuestas con partidos_procesados para ver qué tal nos fue."""
        ...
        if 'Resultado_Real' in df_historial.columns:
            df_historial['Resultado_Real'] = df_historial['Resultado_Real'].astype(object)
        try:
            ...
            if pd.isna(apuesta[match_column]) and apuesta['Recomendacion'] != 'No Bet':
                ...
                if not partido.empty:
                    ...
                    if apuesta['Recomendacion'] == 'H':
                        if resultado == 'H':
                            ganancia = (apuesta['Cuota'] - 1) * apuesta['Kelly_Stake'] * 100 # base 100
                        else:
    ...
    def obtener_metricas():
        if not os.path.exists(HISTORIAL_FILE):
            return {"Aciertos": 0, "Fallos": 0, "WinRate": 0, "ROI": 0, "Yield": 0, "Beneficio_Total": 0}
        ...
        yield_pct = (beneficio_total / inversion_total) * 100 if inversion_total > 0 else 0

        return {
            "Aciertos": aciertos,
            "Fallos": fallos,
            "WinRate": round(win_rate, 2),
            "ROI": round(yield_pct, 2), # En apuestas el ROI y Yield suelen ser intercambiables, pero se usa Yield para medir rentabilidad sobre apostado.
