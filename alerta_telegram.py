...
def enviar_mensaje_telegram(mensaje):
    ...
    if TELEGRAM_TOKEN == "TU_TOKEN_AQUI" or TELEGRAM_CHAT_ID == "TU_CHAT_ID_AQUI":
        logging.warning("⚠️ Credenciales de Telegram no configuradas. Usa variables de entorno o edita alerta_telegram.py")
        return False
    ...
    try:
        ...
        if respuesta.status_code == 200:
            ...
            logging.error(f"❌ Error al enviar Telegram (Http {respuesta.status_code}): {respuesta.text}")
            return False
...
def analizar_y_notificar():
    ...
    ev_picks = df_pred[df_pred['Recomendacion'] != 'No Bet'].sort_values('EV', ascending=False)
