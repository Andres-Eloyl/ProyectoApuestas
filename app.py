...
def api_chat():
    ...
    if mensaje in ['/ayuda', 'ayuda', 'hola']:
        ...
        if predicciones.empty:
            predicciones = df[df['Recomendacion'] != 'No Bet'].tail(3)
            if predicciones.empty:
                 return jsonify({"respuesta": "No he encontrado recomendaciones recientes de valor."})
            respuesta = "No hay picks para hoy. Estos son los últimos 3 picks identificados:\n"
        else:
            predicciones = predicciones.sort_values(by='EV', ascending=False).head(3)
        ...
        return jsonify({"respuesta": f"He analizado {total} partidos en total. He acertado {aciertos}, lo que representa un *{precision:.1f}%* de efectividad global."})
        ...
        return jsonify({"respuesta": "Para **soporte técnico**, abre el **Manual Técnico** en el footer o contacta por Email (SLA 24h). Integración a Discord en progreso."})
