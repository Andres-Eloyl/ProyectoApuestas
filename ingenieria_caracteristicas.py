...
def generar_dataset_ml():
    ...
    for i, fila in df.iterrows():
        ...
        for equipo in [local, visita]:
            if equipo not in stats:
                stats[equipo] = {'puntos_hist': [], 'goles_f': [], 'goles_c': [], 'total_pts': 0, 'xg_f': [], 'xg_c': []}
        ...
        stats[local]['xg_f'].append(xg_partido_l); stats[local]['xg_c'].append(xg_partido_v)
        stats[visita]['xg_f'].append(xg_partido_v); stats[visita]['xg_c'].append(xg_partido_l)
    ...
    logging.info(f"Éxito: Se generó 'dataset_final_ml.csv' con {len(df_final)} partidos y 'stats_actuales.json'.")
