                ...
                for _, row in df.iterrows():
                    try:
                        conn.execute('''
                            ...
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                        ''', (
                            row['Liga'], row.get('Div', codigo_liga), str(row['Date']), 
                            row['HomeTeam'], row['AwayTeam'], 
                            int(row['FTHG']), int(row['FTAG']), row.get('FTR', ''), 
                            int(row.get('HS', 0) if pd.notnull(row.get('HS')) else 0),
                            int(row.get('AS', 0) if pd.notnull(row.get('AS')) else 0),
                            int(row.get('HST', 0) if pd.notnull(row.get('HST')) else 0),
                            int(row.get('AST', 0) if pd.notnull(row.get('AST')) else 0),
                            float(row.get('B365H', 0.0) if pd.notnull(row.get('B365H')) else 0.0),
                            float(row.get('B365D', 0.0) if pd.notnull(row.get('B365D')) else 0.0),
                            float(row.get('B365A', 0.0) if pd.notnull(row.get('B365A')) else 0.0),
                            row['Temporada']
                        ))
                        registros_insertados += conn.execute("SELECT changes()").fetchone()[0]
                    except Exception as e:
                        logging.warning(f"Error procesando fila de {nombre_liga} {temp}: {e}")
                
            except Exception as e:
                logging.warning(f"No se pudo descargar {nombre_liga} Temporada {temp} de football-data: {e}")
...
def extraer_champions_y_copas():
    ...
    try:
        logging.info("Iniciando extracción de Champions League y Copas (Origen CSV alternativo anti-bloqueo)...")
        ...
        if response.status_code == 200:
            ...
            if 'Home Team' in df.columns and 'Away Team' in df.columns and 'Result' in df.columns:
                    ...
                    try:
                        ...
                        if len(parts) == 2:
                            ...
                            conn.execute('''
                                ...
                                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                            ''', ('Champions League', 'UCL', date, home, away, fthg, ftag, ftr, '2324'))
                            registros += conn.execute("SELECT changes()").fetchone()[0]
                    except Exception as parse_e:
                        continue 
                        
                logging.info(f"Éxito: {registros} partidos de Champions League insertados en DB desde nuevo origen.")
        else:
            logging.warning(f"No se pudo descargar el CSV de UCL (Http {response.status_code})")
    except Exception as e:
